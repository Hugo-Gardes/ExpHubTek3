...
uint8_t olc6502::get_flag(FLAGS6502 f)
{
    return ((status & f) > 0) ? 1 : 0;
}

void olc6502::set_flag(FLAGS6502 f, bool v)
{
    if (v)
        status |= f;
    else
...
std::ostream &operator<<(std::ostream &o, olc6502 &c)
{
    o << "STATUS: N" << (c.status & c.N ? "1" : "0");
    o << " V" << (c.status & c.V ? "1" : "0");
    o << " -" << (c.status & c.U ? "1" : "0");
    o << " B" << (c.status & c.B ? "1" : "0");
    o << " D" << (c.status & c.D ? "1" : "0");
    o << " I" << (c.status & c.I ? "1" : "0");
    o << " Z" << (c.status & c.Z ? "1" : "0");
    o << " C" << (c.status & c.C ? "1" : "0") << std::endl;

    o << "PC: $" << std::hex << (uint32_t)c.pc << std::endl;
    o << "A: $" << std::hex << (uint32_t)c.a << std::endl;
    o << "X: $" << std::hex << (uint32_t)c.x << std::endl;
    o << "Y: $" << std::hex << (uint32_t)c.y << std::endl;
    o << "SP: $" << std::hex << (uint32_t)c.stkp << std::endl;
